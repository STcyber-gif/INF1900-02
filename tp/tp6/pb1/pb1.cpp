...
// ESSAYEZ LES DEUX VERSIONS - Commentez/décommentez selon votre circuit
bool isButtonPressed() {
    // VERSION 1: PULL-UP (signal LOW quand pressé)
    // Décommentez celle-ci si votre circuit a la résistance vers VCC
    /*
    ...
    */
    ...
    // VERSION 2: PULL-DOWN (signal HIGH quand pressé)
    // Décommentez celle-ci si votre circuit a la résistance vers GND
    if (PIND & (1 << BUTTON_PIN)) {
        _delay_ms(10);
        if (PIND & (1 << BUTTON_PIN)) {
            return true;
